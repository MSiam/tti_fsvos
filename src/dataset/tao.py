...
class Tao:
    ...
    def get_catid_from_name(self, cat_name):
        reverse_class_mapping = {v: k for k, v in self.class_mapping.items()}
        if cat_name in reverse_class_mapping:
            cat_name = reverse_class_mapping[cat_name]

        for cat_id, cat_info in self.cats.items():
            if cat_info['name'] == cat_name:
                return cat_id
        return -1

    def get_name_from_id(self, class_id):
        #cat_name = self.cats[class_id]['synset'][:-5]
        cat_name = self.cats[class_id]['name']
        if cat_name in self.class_mapping:
            return self.class_mapping[cat_name]
        else:
            return cat_name
    ...
    def get_img_ids_by_vid_ids(self, vid_ids):
        video_images = []
